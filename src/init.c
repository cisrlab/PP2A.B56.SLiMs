...
//TODO: how large should these limits be?
#define DEMIN 2
#define NZMIN 2
...
)
{
  ...
  char *np = NULL;					// value of the -np parameter
  bool sampling = true;					// sampling is on by default
  bool mpi = false;					// not using MPI
  char *pp = NULL;					// used to silently pass the number np of processors
  ...
  *output_dirname = default_output_dirname;
  ...
  DO_STANDARD_COMMAND_LINE(1,
    ...
    NON_SWITCH(1, <dataset> \t\tfile containing sequences in FASTA format\n,
     ...
     DATA_OPTN(2, pp, <pp>, \t\tDO NOT USE (set by exec_parallel), pp = _OPTION_);
  ...
  if (TEXT_ONLY == true) {
    ...
    *text_output = stdout;
    ...
    if (!no_print) {
      ...
      *text_output = fopen(path, "w"); //TODO CEG check for errors
      myfree(path);
    }
  }
#ifdef PARALLEL
